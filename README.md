...
### For Windows:
1) Click on 'Releases' and look for a release that has a text written "Latest" in green color near the title.
...
2) On that release, click on Source code (zip) or Source code (tar.gz) to download.
...
5) Select all two files and a folder named 'assets' from the 'build-version-for-windows' folder. Then drag them into the 'NEET_Girl_Date_Night-1.1-win' folder.
...
6) Then run 'NEET_Girl_Date_Night_Random_Dialogue_Generator_Mod.exe' file.
